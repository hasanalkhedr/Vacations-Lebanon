        ...
        <tbody>
            @foreach ($employees as $employee)
                <tr class="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td class="border px-2 py-4 sm:text-sm font-bold text-gray-900 whitespace-nowrap dark:text-white">
                        <div>
                    ...
                    @foreach($dates as $date)
                        ...
                        @elseif(array_key_exists($employee->id . '&' . $date->format('Y-m-d'), $leaveId_dates_pairs))
                            @if($leaveId_dates_pairs[$employee->id . '&' . $date->format('Y-m-d')]->employee->id == $employee->id)
                                ...
                                @endif
                            @else
                                <td class="border py-4 px-2 text-gray-900 whitespace-nowrap dark:text-white">
                                </td>
                            @endif
                        @else
                            <td class="border py-4 px-2 text-gray-900 whitespace-nowrap dark:text-white">
