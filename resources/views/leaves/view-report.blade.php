<x-sidebar>
    ...
    <div class="overflow-x-auto relative shadow-md">
        <div class="m-4">
            <div class="text-lg">
                <span class="font-bold blue-color">{{__("Name") }}:</span> {{ $employee->first_name }} {{ $employee->last_name }}
            </div>
            <div class="text-lg">
                <span class="font-bold blue-color">{{__("Department") }}:</span> {{ ucfirst(strtolower($employee->department->name)) }}
            </div>
            <div class="text-lg">
                <span class="font-bold blue-color">{{__("Remaining Balances") }}:</span> {{ $employee->nb_of_days }} {{__("Remaining Leaves")}} | {{ $employee->confessionnels }} {{__("Remaining Confessionnels")}}
            </div>
        </div>
                        ...
                        <td class="border-b py-4 px-6 text-gray-900 whitespace-nowrap">
                            <div>
                                @if($leave->substitute_employee)
                                    {{ $leave->substitute_employee->first_name }} {{ $leave->substitute_employee->last_name }}
                                @else
                                    -
                                @endif
                            </div>
