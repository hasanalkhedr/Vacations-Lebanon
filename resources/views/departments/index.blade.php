<x-sidebar>
    ...
    <table class="w-full table-auto rounded-sm border border-gray-300">
        <thead>
        <tr class="border-b-2">
            <th class="py-4 text-lg text-center">
            ...
            <th colspan="2" class="py-4 text-lg text-center">
                <div class="text-blue-400 px-6 py-2">
                    Actions
                </div>
            </th>
        ...
        @unless($departments->isEmpty())
            @foreach ($departments as $department)
                <tr class="border-t border-b border-gray-300">
                    <td class="w-4/12 px-4 py-4 text-lg text-center" onclick="window.location.href='{{ route('departments.show', ['department' => $department]) }}'">
                        <div class="text-blue-400 px-6 py-2">
                            {{ $department->name }}
                        </div>
                    </td>
                    ...
                    <td class="w-1/6 px-4 py-4 text-lg text-center">
                            ...
                            <form method="POST" action="{{ route('departments.destroy', ['department' => $department->id]) }}">
                                ...
                                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                    <div class="sm:flex sm:items-start">
                                        ...
                                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                            <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                                Delete Department: <span class="font-bold">{{ $department->name }}</span>
                                            </h3>
                                            <div class="mt-2">
                                                <p class="text-sm text-gray-500">
                                                    Are you sure you want to delete the department? This action cannot be undone.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
